...
AFogManager::AFogManager()
{
	...
	//메시, 머테리얼 블루프린트에서 넣음
// 	ConstructorHelpers::FObjectFinder<UStaticMeshComponent> tempMeshAlive(TEXT("StaticMesh'/Engine/BasicShapes/Sphere.Sphere'"));
// 	if (tempMeshAlive.Succeeded())
// 	{
// 		compMeshAlive = tempMeshAlive.Object;
// 	}
...
{
	TArray<AActor*> actors; //지역변수. 아웃라이너에 있는 '모든 액터'의 배열 ★★★필요한 것인지 잘 모르겠다
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), ACharacter::StaticClass(), actors); //찾고자 하는 클래스를 상속한 액터를 모두 다 찾아준다

	for (int32 i = 0; i < actors.Num(); i++)
	{
		actorPos = actors[i]->GetActorLocation(); //액터들의 위치
